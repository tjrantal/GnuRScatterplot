...
for (j in 1:length(xVariables)){
	for(i in 1:length(yVariables)){			#Loop to going through the yVariables
		...
		for (g in 1:length(groupedData)){
			...
			text(textXpos[2] ,yLimits[3]+(yLimits[4]-yLimits[3])*0.05+(yLimits[4]-yLimits[3])*0.1*(g-1),substitute(paste(yc,R^2,ya,", ",yb,sep=""),list(ya=RText,yb=PText,yc=groupSymbols[g])),pos=3,offset=0,col = pointColor[g+1]);
		}
	dev.off();
	}
}